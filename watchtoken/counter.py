...
class TokenCounter:
    ...
    def _create_adapter(self, model: str) -> BaseAdapter:
        ...
        if config.provider == ModelProvider.OPENAI:
            ...
            raise UnsupportedModelError(f"No adapter available for provider: {config.provider}")
    
    @classmethod
    def register_adapter(cls, model_name: str, adapter_class: Type[BaseAdapter]) -> None:
