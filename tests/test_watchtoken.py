...
class TestTokenCounter:
    ...
    def test_limit_exceeded_exception(self):
        ...
        with pytest.raises(TokenLimitExceededError):
            tc.check_limit("This text is definitely longer than one token", raise_on_exceed=True)
    ...
    def test_custom_adapter_registration(self):
        ...
        add_model_config("test-model", ModelConfig(
            name="test-model",
...
class TestIntegration:
    ...
    def test_token_counter_with_file_logger(self):
        ...
        with tempfile.NamedTemporaryFile(mode='w', delete=False, suffix='.json') as tmp:
            ...
            with open(tmp.name, 'r') as f:
                data = json.load(f)
                assert len(data["token_usage"]) == 2  # count() and estimate_cost() both call count()
                assert len(data["cost_estimations"]) == 1
